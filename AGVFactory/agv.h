...
#define a_leftx -800*0.2
#define a_lefty -800*0.2
#define a_wight 1600*0.2
#define a_hight 1600*0.2
...
class AGV : public QObject,public QGraphicsRectItem
{
    ...
    Q_PROPERTY(QString _comstate READ ReadCommandState)

    Q_PROPERTY(int  _jiaosudu READ get_jiaosudu)
    Q_PROPERTY(int  _xiansudu READ get_xiansudu)
    Q_PROPERTY(bool  _commstata READ communicateState)
public:
...
public:
    ...
    AGV(QGraphicsScene *scene, TopoView* topoview,qreal zoom,QPointF coordinatemove, int type, QString name, int id, QByteArray address, int startNode,int chargeFullLimit, int chargeUpLimit,int chargeDownLimit, int chexing,int rotate=0,QPointF center=QPointF());
    ~AGV();
    ...
    QString ReadCommandState(){  return Command_state; }
    bool    communicateState(){return  commState;}//通信状态部分未更改//
...
public:
    ...
    void CommandFinish(AGVCommand com);//动作指令完成，向车辆发送完成应答
    void CommandRecord(AGVCommand com1,AGVCommand com2,int i);//动作指令发送，完成日志
    ...
    bool errorCodeToString(QString &error_info);  //获取错误信息输出，无错误返回true
     QMap<int,QList<int>>m_AGVverInArea;
      ...
      int AngleChushi;
      qreal iconzoom;
      AGVCommand Command1;//当前发送的指令1
      AGVCommand Command2;//当前发送的指令2
      AGVCommand ComFinish;//当前完成的指令
      AGVCommand ComDoing;//当前完成的指令

      bool  Chushihua;//初始化的时候，emit一下state_change()，保证车辆循环接收指令
      bool Trafficmanager;
      bool m_bIsPathOK;
      bool TestModel;//表示现在处于测试模式
      bool StateWait;

     int chushidian;//初始点问题修改回来
...
//    quint64 carID_PT20;
 //   quint64 carID_PS20;
    ...
    QString Command_state;//将当前AGV执行动作指令信息读取到agv信息栏
    bool commState;
    int xiansudu;
    int jiaosudu;
signals:
    ...
    void TestFeedback(AGVCommand com,int dongzuo,int fangxiang,int agvid);//为独立测试窗口反馈数据
    void dianliangjinggao();//电量警告
    int  AGVWarn(int WarnID);
...
public slots:
    ...
    void recvData(char frame_type,QByteArray data,bool sign);
