...
MissionManager::MissionManager(QWidget *parent) : QWidget(parent)
{
 ...
 Bsong=false;
   ...
   // --------------------------------------
    ...
    setMaximumSize(1500,560);
    setMinimumSize(1500,560);
    createTableWidget();
    createMissionDialog();
...
void MissionManager::createTestDialog()
{
    ...
    Chushihua=new QPushButton("初始化回应",m_testDlg);

    LeftChuhuo=new QPushButton("左侧出货",m_testDlg);
    RightChuhuo=new QPushButton("右侧出货",m_testDlg);
    ForwardChuhuo=new QPushButton("前伸出货",m_testDlg);
    BackChuhuo=new QPushButton("后伸出货",m_testDlg);

    LeftShouhuo=new QPushButton("左侧收货",m_testDlg);//左收货
    RightShouhuo=new QPushButton("右侧收货",m_testDlg);//右收货
    ForwardShouhuo=new QPushButton("前伸收货",m_testDlg);//前收货
    BackShouhuo=new QPushButton("后伸收货",m_testDlg);//后收货
     ...
     LeftChuhuo->setEnabled(false);//左出货
     RightChuhuo->setEnabled(false);//右出货
     ForwardChuhuo->setEnabled(false);//前出货
     BackChuhuo->setEnabled(false);//后出货

     LeftShouhuo->setEnabled(false);
     RightShouhuo->setEnabled(false);
     ForwardShouhuo->setEnabled(false);
     BackShouhuo->setEnabled(false);
    ...
    semLayout->addWidget(LeftChuhuo);
    semLayout->addWidget(RightChuhuo);
    semLayout->addWidget(ForwardChuhuo);
    semLayout->addWidget(BackChuhuo);


    thirdLayout->addWidget(new QLabel("发送动作:"));
    thirdLayout->addWidget(SendCommand);
    thirdLayout->addWidget(new QLabel("收到动作:"));
    thirdLayout->addWidget(RecvCommand);

    Layout4->addWidget(LeftShouhuo);
    Layout4->addWidget(RightShouhuo);
    Layout4->addWidget(ForwardShouhuo);
    Layout4->addWidget(BackShouhuo);

    FinalLayout->addLayout(mainLayout);
    FinalLayout->addLayout(semLayout);
    FinalLayout->addLayout(Layout4);
    FinalLayout->addLayout(thirdLayout);

    m_testDlg->setLayout(FinalLayout);
   ...
   connect(LeftChuhuo,SIGNAL(clicked()),this,SLOT(action_LeftChuhuo()));
   connect(RightChuhuo,SIGNAL(clicked()),this,SLOT(action_RightChuhuo()));
   connect(ForwardChuhuo,SIGNAL(clicked()),this,SLOT(action_ForwardChuhuo()));
   connect(BackChuhuo,SIGNAL(clicked()),this,SLOT(action_BackChuhuo()));

   connect(LeftShouhuo,SIGNAL(clicked()),this,SLOT(action_LeftShouhuo()));
   connect(RightShouhuo,SIGNAL(clicked()),this,SLOT(action_RightShouhuo()));
   connect(ForwardShouhuo,SIGNAL(clicked()),this,SLOT(action_ForwardShouhuo()));
   connect(BackShouhuo,SIGNAL(clicked()),this,SLOT(action_BackShouhuo()));
...
void MissionManager::createTableWidget()
{
    ...
    m_tableMission->setMaximumSize(1400,390);
    m_tableMission->setMinimumSize(1400,390);
    int x = m_tableMission->geometry().x();
    int y = m_tableMission->geometry().y();
    m_tableMission->setGeometry(x,y,1320,390);
...
bool MissionManager::getNextMission(QString AGVName, Mission &mission, AGV*agv)
{
   ...
   m_sqlTableModel->setFilter(QString("state='%1' and AGV='%2'").arg(MISSION_STATE_WAIT).arg(agv->get_id()));
...
bool MissionManager::open_database()
{
    ...
    m_query.exec("create table if not exists tb_mission "
               "(MissionID int primary key,startNodeID int,startNodeLayer int,endNodeID int,"
               "endNodeLayer int,AGV text,priority int,startTime text,endTime text,Time text,state text,AGVType )");
...
void MissionManager::showAllRecord()
{
    ...
    m_sqlTableModel->setHeaderData(1,Qt::Horizontal,"起始点");
    m_sqlTableModel->setHeaderData(2,Qt::Horizontal,"起点层级");
    m_sqlTableModel->setHeaderData(3,Qt::Horizontal,"终止点");
    m_sqlTableModel->setHeaderData(4,Qt::Horizontal,"终点层级");
    m_sqlTableModel->setHeaderData(5,Qt::Horizontal,"AGV");
    m_sqlTableModel->setHeaderData(6,Qt::Horizontal,"优先级");
    m_sqlTableModel->setHeaderData(7,Qt::Horizontal,"起始时间");
    m_sqlTableModel->setHeaderData(8,Qt::Horizontal,"终止时间");
    m_sqlTableModel->setHeaderData(9,Qt::Horizontal,"用时(S)");

    m_sqlTableModel->setHeaderData(10,Qt::Horizontal,"状态");
    m_sqlTableModel->setHeaderData(11,Qt::Horizontal,"车型");
    m_sqlTableModel->setHeaderData(12,Qt::Horizontal,"货类");
    m_sqlTableModel->setHeaderData(13,Qt::Horizontal,"起始层");
     m_sqlTableModel->setHeaderData(14,Qt::Horizontal,"终止层");
    m_sqlTableModel->select();
    m_tableMission->setModel(m_sqlTableModel);
    m_tableMission->setColumnWidth(0,120);
    m_tableMission->setColumnWidth(1,80);
    m_tableMission->setColumnWidth(2,80);
    m_tableMission->setColumnWidth(3,80);
    m_tableMission->setColumnWidth(4,80);
    m_tableMission->setColumnWidth(5,80);
    m_tableMission->setColumnWidth(6,80);
    m_tableMission->setColumnWidth(7,160);
    m_tableMission->setColumnWidth(8,160);
    m_tableMission->setColumnWidth(9,160);

    m_tableMission->setColumnWidth(10,80);
    m_tableMission->setColumnWidth(11,80);
    m_tableMission->setColumnWidth(12,80);
    m_tableMission->setColumnWidth(13,80);
    m_tableMission->setColumnWidth(14,80);
...
void MissionManager::loadTopoView(TopoView* topoview)
{
    ...
    foreach(TopoVer* ver,m_topoView->m_verList.values()){
        if(ver->is_callPoint()){
            m_comboStart->addItem(ver->get_name(),ver->get_id());
            m_comboEnd->addItem(ver->get_name(),ver->get_id());

        }
         Targetnode->addItem(QString("%1").arg(ver->get_id(),4,16,QLatin1Char('0')),ver->get_id());
...
void MissionManager::action_create()
{
    ...
    if(m_missionDlg->exec()){
        ...
        record.setValue("chexing",m_comboAGV->currentData().toString());//当前是按照车型下发任务
 ...
 if(direction==true)
   {
           ...
           SendCommand->append(QString("[%1],agv=%2,start=%3,end=%4,length=%5,angle=%6,Direction=%7,Action=%8").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel).arg("前进").arg("正常"));
  ...
  // Tingche->setEnabled(false);
   ...
   LeftChuhuo->setEnabled(false);
   RightChuhuo->setEnabled(false);
   ForwardChuhuo->setEnabled(false);
   BackChuhuo->setEnabled(false);
  ...
  if(direction==true)
    {
            ...
            com.test=true;
            qDebug()<<"测试程序距离："<<(uint)distance<<endl;
            SendCommand->append(QString("[%1] agv=%2,start=%3,end=%4,length=%5,angle=%6,Direction=%7,Action=%8").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel).arg("前进").arg("减速"));
            agv->set_targetCommand(com,com1);
  ...
  if(direction==true)
    {
            ...
            com.test=true;
            qDebug()<<"测试程序距离："<<(uint)distance<<endl;
            SendCommand->append(QString("[%1] agv=%2,start=%3,end=%4,length=%5,angle=%6,Direction=%7,Action=%8").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel).arg("后退").arg("正常"));
            agv->set_targetCommand(com,com1);
  ...
  if(direction==true)
    {
            ...
            com.test=true;
            qDebug()<<"测试程序距离："<<(uint)distance<<endl;
            SendCommand->append(QString("[%1]  agv=%2,start=%3,end=%4,length=%5,angle=%6,Direction=%7,Action=%8").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel).arg("后退").arg("减速"));
            agv->set_targetCommand(com,com1);
...
void MissionManager::action_LeftChuhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
    ...
    {
        SendCommand->append(QString("[%1] agv=%2,start=%3,end=%4,length=%5,angle=%6 左转出货").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel));
        agv->set_targetCommand(com,com1);
    }
...
void MissionManager::action_RightChuhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
    ...
    {
        SendCommand->append(QString("[%1] agv=%2,start=%3,end=%4,length=%5,angle=%6 右转出货").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel));
        agv->set_targetCommand(com,com1);
    }
...
void MissionManager::action_ForwardChuhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
    ...
    {
        SendCommand->append(QString("[%1] agv=%2,start=%3,end=%4,length=%5,angle=%6 前伸出货").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel));
        agv->set_targetCommand(com,com1);
    }
...
void MissionManager::action_BackChuhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
    ...
    {
        SendCommand->append(QString("[%1] agv=%2,start=%3,end=%4,length=%5,angle=%6 后伸出货").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agv->get_id()).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel));
        agv->set_targetCommand(com,com1);
    }
...
void MissionManager::action_LeftShouhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
...
void MissionManager::action_RightShouhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
...
void MissionManager::action_ForwardShouhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
...
void MissionManager::action_BackShouhuo()
{
    AGVCommand com,com1;
    AGV*agv;
    if(m_agvId1.contains(TestAGV->currentData().toInt()))
...
void MissionManager::action_recvCommand(AGVCommand com,int Dongzuo,int Fangxiang,int agvid)
{
    ...
    else if((Fangxiang==0x01)&&(Dongzuo==0x09))
    {
        fangxiang="前伸";
        dongzuo="出货";

    }
    else if((Fangxiang==0x02)&&(Dongzuo==0x09))
    {
        fangxiang="后伸";
        dongzuo="出货";

    }
    else if((Fangxiang==0x04)&&(Dongzuo==0x09))
    {
        fangxiang="左转";
        dongzuo="出货";

    }
    else if((Fangxiang==0x08)&&(Dongzuo==0x09))
    {
        fangxiang="右转";
        dongzuo="出货";

    }
    else if((Fangxiang==0x01)&&(Dongzuo==0x08))
    {
        fangxiang="前伸";
        dongzuo="收货";

    }
    else if((Fangxiang==0x02)&&(Dongzuo==0x08))
    {
        fangxiang="后伸";
        dongzuo="收货";

    }
    else if((Fangxiang==0x04)&&(Dongzuo==0x08))
    {
        fangxiang="左转";
        dongzuo="收货";

    }
    else if((Fangxiang==0x08)&&(Dongzuo==0x08))
    {
        fangxiang="右转";
        dongzuo="收货";
    ...
    {
       RecvCommand->append(QString("[%1] agv=%2,初始化完成,初始节点=%3，初始车型=%4,初始位姿=%5").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agvid).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.angel));
      ...
      if(Chushihua==false)
      {
            ...
            Tingche->setEnabled(true);
             ...
             LeftChuhuo->setEnabled(true);
            RightChuhuo->setEnabled(true);
            ForwardChuhuo->setEnabled(true);
            BackChuhuo->setEnabled(true);
            Tingche->setEnabled(true);

            LeftShouhuo->setEnabled(true);
            RightShouhuo->setEnabled(true);
            ForwardShouhuo->setEnabled(true);
            BackShouhuo->setEnabled(true);
    ...
    RecvCommand->append(QString("[%1] agv=%2,start=%3,end=%4,length=%5,angle=%6,Direction=%7,Action=%8").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")).arg(agvid).arg(com.start,4,16,QLatin1Char('0')).arg(com.end,4,16,QLatin1Char('0')).arg(com.length).arg(com.angel).arg(fangxiang).arg(dongzuo));
