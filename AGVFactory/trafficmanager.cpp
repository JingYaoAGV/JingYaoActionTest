...
void TrafficManager::updateGlobalPlanning()
{
    ...
    __log__(&m_file,QString("agvid: %1 current_node: %2 ,angle：%3").arg(agvid).arg(current_node).arg(agv->agvAngel));
...
*/
    ...
    if(!m_mapAGVandMission.contains(agv_name)){  // 当前agv无任务
        ...
        if(agv->get_Commandnumber()!=0)//1,无任务，循环检测，2，回到初始点，先做完当前指令
             {
...
void TrafficManager::CommandProduce(int current_node,QList<int> path,QList<AGVCommand>&Command,int angel,QString agv_name)
{
    ...
    for (int i=0;i<=Command.size()-1;i++)
    {
       ...
       switch(Command.at(i).Dongzuo)
        ...
        __log__(&m_file,QString("agvname: %1 CommandPlanning/ Command:startnode:%2 endnode:%3 Direction:%4 Action:%5 Angle:%6 Length:%7 ").arg(agv_name).arg(Command.at(i).start).arg(Command.at(i).end).arg(Direction).arg(Action).arg(Command.at(i).angel).arg(Command.at(i).length));
