...
AGV::AGV(QGraphicsScene *scene, TopoView* topoview, qreal zoom,QPointF coordinatemove,int type,QString name, int id, QByteArray address,int startNode,int chargeFullLimit,int chargeUpLimit,int chargeDownLimit,int chexing,int rotate,QPointF center )
{
    ...
    TestModel=true;//此版本为单车调试版本，测试单车运行
...
void AGV::CommandRecord(AGVCommand com1, AGVCommand com2, int i)
{
    if(i==1)
 {
  ...
  __log__(&m_file,QString("[send]agv=%1 send path command: C1 number=%2,start=%3 end=%4 %5 %6 length %7 angle %8").arg(m_nID).arg(com1.number).arg(com1.start).arg(com1.end).arg(Fangxiang1).arg(Dongzuo1).arg(com1.length).arg(com1.angel));
  __log__(&m_file,QString("[send]agv=%1 send path command: C2 number=%2,start=%3 end=%4 %5 %6 length %7 angle %8").arg(m_nID).arg(com2.number).arg(com2.start).arg(com2.end).arg(Fangxiang2).arg(Dongzuo2).arg(com2.length).arg(com2.angel));
    ...
    if(i==2)
 {
  ...
  __log__(&m_file,QString("[recv]agv=%1 Path Command Finish: C1 number=%2 start=%3 end=%4 %5 %6  length %7 angle %8").arg(m_nID).arg(com1.number).arg(com1.start).arg(com1.end).arg(Fangxiang1).arg(Dongzuo1).arg(com1.length).arg(com1.angel));
...
void AGV::recvData(char frame_type, QByteArray data, bool sign)
{
    ...
    if(((uchar)frame_type)==0xf2)
    {
        ...
        {     m_bIsPathOK=true;
            qDebug()<<"AGV"<<m_nID<<"路径发送收到"<<endl;
            __log__(&m_file,QString("[recv] agv=%1,Send Path Answer:number_1=%2,number_2=%2").arg(m_nID).arg(number1).arg(number2));
    ...
    else if(frame_type==FRAME_ENABLE_PATH || frame_type==FRAME_ENABLE_MULTIMODE_PATH)
    {
       ...
       if(agvcommand.contains(number))
      {
            ...
            Command_state=QString("targetnode:%1 action:%2,direction:%3").arg(ComDoing.end).arg(ComDoing.dongzuo).arg(ComDoing.fangxiang);
      }
...
//        m_bIsErrorOK = true;
    ...
    else if(((uchar)frame_type)==0xf1)//初始化命令
    {
        ...
        __log__(&m_file,QString("[recv]agv=%1 Initialization: InitialPoint=%2").arg(m_nID).arg(m_nTopoPosID));
        //暂未定义状态
