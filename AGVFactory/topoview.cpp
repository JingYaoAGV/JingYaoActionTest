...
TopoVer* TopoView::add_topoVer(QPointF center,int sita,int tunnel,int id,int radius, QColor color, bool is_callPoint)
{
    ...
    */
    ...
    while(query.next()){
        ...
        add_topoVer(QPointF(posx,posy),sita,
                    isInTunnel,id,5,brush.color(),type);
...
TopoVer::TopoVer(QGraphicsScene *scene,int sita,int id,int tunnel,QPointF center, int radius, int ver_type)
{
   ...
   // setFlag(QGraphicsItem::ItemIsMovable);
    ...
    isInTunnel=tunnel;
    m_nSita=sita;
    m_labelItem = new QGraphicsSimpleTextItem(QString("%1").arg(m_nID,4,16,QLatin1Char('0')));
